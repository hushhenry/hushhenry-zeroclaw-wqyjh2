//! OpenAI Codex OAuth (ChatGPT session).
...
impl OAuthProvider for OpenAiCodexOAuthProvider {
    ...
    async fn login(&self, callbacks: &dyn OAuthCallbacks) -> anyhow::Result<OAuthCredentials> {
        ...
        callbacks.on_auth(OAuthAuthInfo {
            url: auth_url,
            instructions: Some(
                "A browser should open. If not, visit the URL and paste the redirect URL here."
                    .into(),
            ),
        });
        ...
        let expires =
            chrono::Utc::now().timestamp_millis() + token.expires_in * 1000 - 300_000;
